...
def detect_centroids(frame_gray, minArea, maxArea, blobColor, filter_type,
...
def kalman_tracking_video(
    ...
    def detect(gray):
        return detect_centroids(gray, minArea, maxArea, blobColor, filter_type,
                                   filterByCircularity, minCircularity,
                                   filterByInertia, filterByConvexity, filterByColor,
                                   **det_kw)
...
def _save_csv(det_pos, pixel_size, csv_mode, csv_path):
    """
    csv_mode:
      'single'    -> one file. Columns: track_id, frame, x_px, y_px, x_um, y_um
      'per_track' -> one CSV per track saved in folder csv_path.
    """
    if csv_mode == 'single':
